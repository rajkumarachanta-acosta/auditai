...
export default async function PainPointsPage({
  ...
  return (
    <div>
      <PageHeader title="Pain Points" description="Every extracted pain point, searchable and filterable." />
      <div className="space-y-4 px-8 py-6">
        <PainPointFilterBar action="/reachy/pain-points" categories={categories} current={currentParams} />
        <PainPointTable rows={rows} />
        <Pagination page={page} pageSize={pageSize} total={total} action="/reachy/pain-points" params={currentParams} />
      </div>
    </div>
  );
}
